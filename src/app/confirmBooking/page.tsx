...
const ConfirmBooking = () => {
  ...
  // const seatCounter = useSelector((state: RootState) => state.counter.seatCounter);
  ...
  const publicSessionPrice = useSelector(
    (state: RootState) => state.counter.publicSessionPrice,
  );

  // useEffect(()=>{
  //   //console.log("Invitees from redux",Invitees);
  // },[Invitees]);
  ...
  const payByUPI = ["PhonePe UPI", "Google Pay"];
  ...
  // const finalPrize = sessionPrize && sessionDiscount ? (sessionPrize * (1 - sessionDiscount / 100)) as number : sessionPrize;
  const finalPrize = 300;
...
async function handlePaymentFlow(event:any) {
  event.preventDefault();
  console.log("handlePaymentFlow initiated");

  // Invoke handlePaymentMode to validate payment selection
  handlePaymentMode();

  // Exit if there's an error from handlePaymentMode
  if (err) {
    console.log("Error in payment mode selection:", err);
    return;
  }
  ...
  try {
    ...
    if (!sessionId) {
      console.error("sessionId not found in URL parameters");
      return;
    }
    ...
    const options = {
      key: "rzp_test_99Mi4G9l7ZajTw",
      amount,
      currency,
      name: "FitnEarn",
      description: "Public Session Payment",
      image: "https://example.com/your_logo",
      order_id: data.id,
      handler: async function (response:any) {
        console.log("Payment success response:", response);

        // Proceed to add participant and record payment details
        try {
          await addParticipants();
          await recordPaymentDetails("Public", sessionId, payMode, response);
        } catch (error) {
...
function handlePaymentMode() {
  console.log("handlePaymentMode function is called");
  if (!selectedValue) {
    setErr("Payment Method is Required");
  } else {
    //console.log("Payment Method :", selectedValue);
    if (selectedValue === "Pay by UPI" && !selectedUPI) {
      setErr("Please choose UPI method for a payment");
      return;
    }
    if (selectedValue === "Credit & Debit Cards" && !selectedCard) {
      setErr("Please choose card for a payment");
      return;
    }
    if (selectedValue === "More Payment Options" && !selectedStore) {
      setErr("Please choose store for a payment");
      return;
    }
    if (selectedValue === "Wallets" && !selectedWallet) {
      setErr("Please choose Wallet for a payment");
      return;
    }
    setErr("");
    // addParticipants();
...
async function addParticipants() {
  ...
  try {
    const response = await fetch(
      `${apiEndpoint}/api/fitnearn/web/users/session/add-participant/${sessionId}`,
      {
        method: "PATCH",
        headers: {
    ...
    console.log("Result from addParticipants function:", result);
    if (result.success) {
      toast({
      ...
      // recordPaymentDetails("Public", sessionId);
    } else {
      // console.log(
      //   "something went wrong while adding participants in Backend",
      // );
    }
  } catch (err) {
    //console.error("Error in addParticipants function:", err);
  }
}
...
const recordPaymentDetails = async (
  ...
  const randomPaymentId = Math.floor(1000 + Math.random() * 9000); // Generate a random 4-digit number
  ...
  try {
    const response = await fetch(
      `${apiEndpoint}/api/fitnearn/web/users/booking/add`,
      {
        ...
        body: JSON.stringify({
          orderId : razorpayResponse.razorpay_order_id || randomOrderId,
          sessionId: session_id,
          paymentId: razorpayResponse.razorpay_payment_id || randomPaymentId,
    ...
    if (result.success) {
      console.log("Payment details recorded successfully:", result);
      setTimeout(() => {
        router.push(`/my_bookings/upcoming_bookings/${session_id}`);
      }, 1000);
    }else{
    console.log("Payment details record result:", result);
    }
  } catch (error) {
  ...
  const handlePrivateSessionPayment = async () => {
    console.log("handlePrivateSessionPayment function is called");
    ...
    const sessionDetails = {
      ...
      participantsEmail: Invitees,
      invitedEmail: requestedInvitees,
      seats: Invitees.length,
      price: sessionPrize,
    };
  ...
  // Format the date as "Wed, May 08"
  const formattedDateForm = new Intl.DateTimeFormat("en-US", {
    weekday: "short",
    month: "short",
    day: "2-digit",
  }).format(currentDate);

  // Format the time as "5:45 pm"
  ...
  const fetchCoachDetails = async () => {
    try {
      const response = await fetch(
        `${apiEndpoint}/api/fitnearn/web/coach/profile/${coachId}`,
      );
      const result = await response.json();
      //console.log(result);
      if (result.success) {
        setCoachDetails(result.data);
      } else {
        //console.log("error in fetching coach details");
      }
    } catch (error) {
    ...
    <div className="pt-[72px] md:pt-[92px] mb-[70px]">
      ...
      <div className="flex md:flex-row flex-col-reverse md:mt-[50px]">
        <div className="flex flex-col items-center flex-1">
          ...
          >
            <div className="flex w-[328px]  md:w-[500px]  pb-4 mt-4 mb-5">
                ...
                <div className="flex gap-6 mt-4">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-[#FFFFFF] text-[16px] md:text-[20px] font-normal leading-normal">
                      {/* {formattedDate ? formattedDate : formattedDateForm} */}
                      {formattedDate ? formattedDate : sessionData?.date}
                    </span>
                  </div>
                  <div className="flex items-center justify-center gap-2">
                    <svg
                    ...
                    <span className="text-[#FFFFFF]  text-[16px] md:text-[20px] font-normal leading-normal">
                      {/* {timeSlot ? timeSlot : formattedTime} */}
                      {timeSlot ? timeSlot : sessionData?.startTime}
                    </span>
                  </div>
                </div>
                <div className="h-[1px] w-full md:w-[480px] bg-[#525252] mt-7"></div>
              </div>
            </div>

            {/* <div className="flex w-[328px]  md:w-[500px]  pb-4 ">
              <div className="text-[20px] md:text-[32px] text-[#FFFFFF] font-bold leading-normal pr-2">
                2.
              </div>
              <div>
        ...
        <div className="flex-1 flex justify-center items-center md:mt-0 mt-5">
          <div className="w-[328px] md:w-[522px] about-coach-sub-div p-[22px] md:p-8">
            <div className="flex justify-start items-center gap-10 md:gap-[53px] pb-6 border-b-[1px] border-[#525252]">
              ...
              <div>
                <h2 className="text-[20px] md:text-[30px] text-[#FFFFFF] font-bold leading-normal">
                  {coachDetails?.name ?? "name"}
                </h2>
                <p className="text-[18px] md:text-[24px] text-[#FFFFFF] font-medium leading-normal">
                  {coachDetails?.categoryLevel ?? "Intermediate"} :{" "}
                  {coachDetails?.category ?? "-"}
                </p>
              </div>
            </div>
            <div className="py-4 md:border-t-[1px] md:border-b-[1px] md:border-[#525252]">
              ...
              <div className="w-full flex justify-between items-center mb-2">
                ...
                <span className="text-[18px] md:text-[20px] text-[#FFFFFF] font-normal leading-normal">
                  {sessionData ? sessionData?.discountPercent : 5} %
                </span>
              </div>
              <div className="w-full flex justify-between items-center mb-2">
                <span className="text-[16px] md:text-[20px] text-[#FFFFFF] font-normal leading-normal">
                ...
                <span className="text-[18px] md:text-[20px] text-[#FFFFFF] font-normal leading-normal">
                  {/* ₹ {sessionPrize ? sessionPrize : "250"} */}₹
                  {sessionType === "Private" && sessionPrize
                    ? sessionPrize
                    : publicSessionPrice}
                </span>
              </div>
            ...
            <div className="text-[#FFFFFF] hidden md:block pt-3">
              ...
              <p className="text-[16px] md:text-[20px] font-normal leading-normal">
                ...
                {sessionData
                  ...
                  : formatDateForCoachCard(formattedDate)}
                , {sessionData ? sessionData?.startTime : timeSlot} (IST).
              </p>
              <Link
      ...
      <div className="w-full flex md:hidden flex-col justify-center items-center ">
        <div className="about-coach-sub-div text-[#FFFFFF] w-[328px]  py-[20px] px-[22px]">
          ...
          <p className="text-[16px] font-normal leading-normal mt-4">
            Get a full refund if you cancel by{" "}
            {formatDateForCoachCard(sessionData?.date)},{" "}
            {sessionData?.startTime} (IST).
          </p>
          <Link
            href={"https://www.fitnearn.com/terms-conditions"}
            rel="noopener noreferrer"
            target="_blank"
            className="text-[18px] font-bold leading-normal underline cursor-pointer"
          >
            Learn More
          </Link>
        </div>
