...
const Page = () => {
  ...
  const CancellationTermsModal = () => {
    return (
      <div className="bg-[#1e1e1e] absolute mq1050:left-40  text-white rounded-lg p-6 w-[800px] h-[625px] font-sans top-36 left-80">
        <div className="mb-4 text-lg">Terms of Cancellation</div>
        <div className="flex justify-center py-2 text-center">
          <Image
            className="rounded-xl "
            width={500}
            height={100}
            src="/cancel_policy.png"
            alt=""
          />
        </div>
        <div className="flex gap-48 mb-6">
          <div className="mb-1 text-base font-bold">
            After <br /> <span className="text-2xl">Cancel</span>
          </div>
          <div>
            <div className="text-xl font-bold">Full refund</div>
        ...
        <div className="flex gap-40 mb-6">
          <div className="mb-1 text-base font-bold">
            After <br /> <span className="text-2xl">24 Hours</span>
          </div>
          <div>
            <div className="text-xl font-bold">No refund</div>
            <div className="mt-1 text-sm">
        ...
        <div className="flex gap-40 mb-6">
          <div className="mb-1 text-base font-bold">
            Amount <br /> <span className="text-2xl">72 Hours</span>
          </div>
          <div>
            <div className="text-xl font-bold">Amount Refunded</div>
            <div className="mt-1 text-sm">
              The amount will be refunded to the users within 72 hours of the
              cancellation
            </div>
          </div>
        </div>
        <div className="absolute bottom-6 left-[600px]">
          <button className="px-4 py-2 text-white bg-gray-700 rounded">
            Cancel
          </button>
        </div>
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        //console.log("Users active subscriptions", data);
      } catch (err) {
  ...
  //repeate the sub plan
  const repeatSubscription = async () => {
    const userId = Cookies.get("user_id");
    if (!userId || !activeSubscription) {
      setError(
        "Unable to repeat subscription. Missing user ID or active subscription.",
      );
      return;
    }
    ...
    try {
      const response = await axios.post(
        `${apiEndpoint}/api/fitnearn/web/users/subscription-plan/subscribe`,
        {
          userId: userId,
          updatedDetails: {
            planId: activeSubscription.subscriptionPlan.plan_id,
            payment_mode: "Credit Card", // You might want to make this dynamic
            transactionId: "12344", // You might want to generate this dynamically
            RepeatPlan: true,
          },
        },
      );
      ...
      if (response.data.success) {
        ...
        setShowRepeat(false);
        // Optionally, refresh the active subscription data
        fetchActiveSubscription(userId);
        //console.log("Plan repeated successfully");
        toast({
          title: "Plan Repeated!",
          description: "Successfully repeated the plan!",
          duration: 5000, // 5 seconds
        }); //route the user to the home page
      } else {
        setError("Failed to repeat subscription. Please try again.");
        toast({
      ...
      //console.error("Error repeating subscription:", err);
      // location.reload();
      ...
      // setError("An error occurred while repeating the subscription.");
      return;
    }
  };
  ...
  const cancelSub = async () => {
    ...
    if (!subId || !activeSubscription) {
      setError(
        "Unable to cancel subscription. Missing user ID or active subscription.",
      );
      return;
    }
    ...
    try {
      const response = await axios.delete(
        `${apiEndpoint}/api/fitnearn/web/users/subscription-plan/cancel-subscription/${subId}`,
      );

      // Handle the response
      if (response.data.success) {
        setShowCancellationTermsModal(false);
        setShowCancel(false);
        setShowNotification(true);
        //console.log(response);
        //console.log("Plan Canclled successfully");
        //console.log();
      } else {
        //console.error("Error Canclled subscription:");
      }
    } catch (err) {
      //console.error("Error Canclled subscription:", err);
    }
  };
  ...
  const plan_id = activeSubscription?.plan_id || "0";
  Cookies.set("plan_id", activeSubscription?.plan_id);
    ...
    return (
      <div>
        <div className="m-auto py-28 ">
          ...
          <div className="flex items-center gap-8 ml-44 mq450:ml-2 mq450:mt-10">
            <div className="flex items-center gap-5 mq450:gap-2 mq450:text-xl border-neutral-500 mq450:w-[340px] mq450:overflow-x-auto mq450:scrollbar-thin">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.path}
  ...
  return (
    <>
      <div className="relative mq450:static ">
        <UserNavbar activeplans={true} plansactivecolor="neutral-700" />
        {data ? (
          <div className="absolute mq450:left-[5px] text-white mq450:ml-2 top-28 left-44  mq450:mt-14 w-fit">
            <div className="flex items-center gap-8">
              ...
              <div className="flex gap-5 mq450:flex-row-reverse mq450:gap-3 mq450:absolute mq450:left-[152px] mq450:top-[50px] z-20">
                <div className="flex gap-5 mq450:flex-row mq450:gap-3 mq450:absolute mq450:left-[10px] mq450:top-[-5px] z-20">
                  {activeSubscription.plan_id != 4 ? (
                    // <Link href="/subscription-plans">
                    <button
                      onClick={checkUpcomingPlanBeforeUpgrade}
                      className="w-[111px] text-white mq450:text-xs  mq450:w-[88px] mq1050:ml-[0px] border hover:border hover:border-neutral-500  mq1240:ml-[12px] bg-gradient-to-r from-[#F43F5E] hover:bg-transparent to-[#FB923C] mq450:ml-[2px] ml-[370px] hover:bg-transparent text-sm text-nowrap h-[33px] px-3 py-1.5 rounded-lg border border-neutral-600 justify-center items-center gap-2 inline-flex"
                    >
                      Upgrade Plan
                    </button>
                  ) : (
            ...
            <div className="mq1240:w-[980px]  mq w-[1220px] mq450:justify-start mq450:flex mq1050:w-[820px] mq450:flex-col mq450:h-auto mq450:gap-3 mq450:items-start mt-7 mq450:w-[348px] gap-32 h-auto relative bg-neutral-800 rounded-xl shadow border border-neutral-700 mq450:mt-7 mq450:top-7">
              {showWarningPopUp && (
                <Popup
              ...
              <div className="flex p-5 pt-8 mq450:pt-5 gap-[340px] mq1240:gap-[225px] mq450:flex-col mq450:gap-2 mq1050:gap-36">
                ...
                <hr
                  style={{ backgroundColor: "neutral-800" }}
                  className="mq450:w-[300px] border-neutral-600 mq450:block hidden"
                />
                <div>
                  <p className="text-xl font-bold mq450:text-kg text-start text-nowrap font-Lato mq450:pt-2">
                    Plan Amount
                  </p>

                  <div className="grid grid-cols-2 gap-1 mt-[16px] text-neutral-200 text-[16px]">
                    <p className="">Total:</p>
                    ...
                    {activeSubscription.previousActivePlanPrice !== 0 && (
                      <>
                        <p className="text-neutral-200">Previous plan:</p>
                        <p className="text-right text-neutral-200">
                          -
                          {formatCurrency(
                            activeSubscription.previousActivePlanPrice,
                          )}
                        </p>
                      </>
                    )}

                    {activeSubscription.previousRepeatedPlanPrice !== 0 && (
                      <>
                        <p className="text-neutral-200 text-nowrap">
                          Repeated plan:
                        </p>
                        <p className="text-right text-neutral-200">
                          -
                          {formatCurrency(
                            activeSubscription.previousRepeatedPlanPrice,
                          )}
                        </p>
                      </>
                    )}
              ...
              <div className="w-full  h-[0px] border border-neutral-700 mq450:"></div>
              <div className="flex   justify-between p-4  gap-[800px] mq1240:gap-[650px] mq1240:pb-2 mq1050:gap-[450px] mq1050:p-4 items-center mq450:flex-row mq450:gap-4 mq450:pt-0 mq1050:p-3">
                <div
                  onClick={() => {
                    setShowRepeat(true);
                  }}
                  className="text-xl bg-clip-text  text-nowrap mq450:text-md mq450:px-1 text-transparent bg-gradient-to-r from-[#F43F5E] to-[#FB923C]  text-2xl font-bold  font-medium cursor-pointer text-nowrap mq450:font-medium text-rose-500 font-Lato"
                >
                  Repeat Plan
                </div>
                {/* <div
                  ...
                  className="text-xl cursor-pointer text-nowrap text-rose-500 font-Lato"
                >
                  Cancel Subscription
                </div> */}
              </div>
            ...
            {showNotification && (
              <div className="absolute flex flex-col items-start justify-between gap-2 p-4 py-6 mx-auto text-white border-neutral-700 border-2 rounded-xl w-[800px] top-10 left-60 bg-neutral-800 mq1050:left-2">
                ...
                <div />
                <p className="text-sm">
                  Your Subscription has been Canceled and amount will be
                  refunded in next 72 hours
                </p>
          ...
          <div className="absolute mq450:left-[8px] text-white mq450:ml-2 top-40 left-44 mq450:left-0 mq450:mt-5 w-fit">
            <div className="flex gap-5 mq450:flex-row-reverse mq450:gap-3 mq450:absolute mq450:left-[160px] mq450:top-[200px] z-20">
              {plan_id != 4 ? (
                // <Link href="/subscription-plans">
                <button
                  onClick={checkUpcomingPlanBeforeUpgrade}
                  className="w-[111px] text-white mq450:text-xs   hover:bg-transparent hover:border-neutral-50 mq450:w-[80px] mq1050:ml-[0px]  mq1240:ml-[12px] bg-gradient-to-r from-[#F43F5E] to-[#FB923C] mq450:ml-[2px] ml-[370px] text-sm text-nowrap h-[33px] px-3 py-1.5 rounded-lg border border-neutral-600 justify-center items-center gap-2 inline-flex"
                >
                  Upgrade Plan
                </button>
              ) : (
              ...
              <button
                ...
                className={` hover:bg-gradient-to-r hover:from-[#F43F5E] hover:to-[#FB923C] text-white mq1050:ml-[00px] mq450:text-xs mq450:w-[80px] mq1240:ml-[00px] hover:bg-gradient-to-l from-pink-600 to-orange-500 mq450:ml-[px] ml-[00px] text-sm text-nowrap h-[33px] px-3 py-1.5 rounded-lg border border-neutral-600 justify-center items-center gap-2 inline-flex ${
                  activeSubscription.plan_id == 4 ? "ml-[500px]" : ""
                }`}
              >
                Raise Ticket
              </button>
            ...
            <div className="flex flex-col items-center justify-center mt-16">
              ...
              <div className="flex flex-col items-center justify-center gap-1 mt-10 text-center">
                ...
                <button className="w-[200px] h-[35px] px-3 py-2 bg-gradient-to-l from-pink-600 to-orange-500 rounded-lg justify-center items-center gap-2 inline-flex">
                  Premium Member Now
                </button>
              </div>
            </div>
          ...
          <div className="bg-[#4d4d4d] justify-center  z-50 w-[580px] h-[397px] mq450:w-[300px] mq450:top-[190px] rounded-xl align-center absolute mq450:left-[30px] left-[450px] top-[152.50px] flex-col justify-start items-center  inline-flex ">
            ...
            <div className="flex inline-flex flex-col items-center justify-center gap-1 ">
              ...
              <p className="text-xl font-semibold text-center mq450:text-[16px] text-neutral-100 font-Lato">
                Are you sure you want to repeat your current  plan?
              </p>
        ...
        {showCancel && (
          <div className="bg-[#4d4d4d] mq1050:left-60 justify-center  w-[580px] h-[397px] mq450:w-[300px] mq450:top-[190px] rounded-xl align-center absolute mq450:left-[30px] left-[450px] top-[152.50px] flex-col  items-center  inline-flex ">
            <svg
              onClick={() => {
                setShowCancel(false);
              }}
              className="ml-[500px] cursor-pointer mq450:ml-56"
              xmlns="http://www.w3.org/2000/svg"
              width="38"
              height="38"
              viewBox="0 0 48 48"
            ...
            <div className="flex inline-flex flex-col items-center justify-center gap-1 ">
              <Image height={100} width={120} src="/mdi_alert.png" alt="" />
              <p className="text-white text-[40px] font-bold font-Lato leading-[60px] pt-">
                Cancel Membership
              </p>
              <p className="text-lg font-semibold text-center text-neutral-100 font-Lato">
                Are you sure you want to cancel your membership?
              </p>
              <p className="text-sm font-semibold text-center underline text-neutral-100 font-Lato">
              ...
              <div className="flex items-start justify-center gap-4 pt-5">
                ...
                <button
                  onClick={() => {
                    setShowCancellationTermsModal(true);
                  }}
                  className="px-4 py-2 text-white transition duration-200 rounded bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600"
                >
                  Confirm
        ...
        {ShowCancellationTermsModal && <CancellationTermsModal />}
      </div>
    </>
  );
};
