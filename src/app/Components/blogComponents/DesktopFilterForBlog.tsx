...
import newBlogHeaderImg from "../../../../public/newerBlogHeader.png";
import Cookies from "js-cookie";
import { BlogCardsRowSkeleton } from "../skeletons/BlogSkeleton";
import workoutBanner from "../../../../public/workoutVideosBanner.png";
import blogNewHeader from "../../../../public/blogNewHeaderr.png";
...
const DesktopFilterForBlog = () => {
  ...
  const totalPages = Math.ceil(
    (filteredBlogs && filteredBlogs).length / itemsPerPage,
  );
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = (filteredBlogs && filteredBlogs).slice(
    startIndex,
    endIndex,
  );
  ...
  async function fetchPopularBlogs() {
    ...
    // console.log("popularBlogs :", popularData);
    ...
    <>
      <div className="hidden md:flex relative w-full h-[522px] justify-center items-center">
        <Image
          ...
          width={1280}
          height={522}
          className="w-[100vw] h-[522px] "
          src={blogNewHeader}
          // src={workoutBanner}
        />
        <div className="absolute w-full max-w-[786px] flex flex-col justify-center items-center">
          <h1 className="text-[72px] w-full max-w-[786px] text-[#F5F5F5] font-bold leading-normal text-center">
            Guide to a healthier you
          </h1>
          <h2 className="text-[32px] w-full max-w-[756px] text-[#E5E5E5] font-regular leading-normal text-center mt-6">
            Explore expert tips, wellness insights, and inspiring stories for
              ...
              >
                <DialogHeader>
                  ...
                  <DialogDescription className="flex flex-col items-center justify-center">
                    {/* <div className="w-full h-[1px] bg-[#a3a3a352]"></div> */}
      ...
      <section className="w-full hidden md:flex flex-wrap justify-center items-start gap-6 xl:gap-[50px] pb-[64px] pt-6 md:pt-[50px] px-4 md:px-[30px] xl:px-[130px]">
        {/* {isLoading ? <BlogCardsRowSkeleton /> : (     
        {
          filteredBlogs.length <= 0 && popularBlogs
          ? popularBlogs.slice(0, 3).map((blog: any) => (
            <Carousel opts={{ align: "start" }} className="w-full flex justify-center items-center max-w-[1000px]  bg-[#171717]">
              <CarouselContent className="-ml-1 bg-[#171717]">
                <CarouselItem key={blog._id}
                  className="md:basis-auto bg-[#171717] pl-[30px]"
                >
                  <div
                    key={blog._id}
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center justify-center gap-2">
                              <Image
                                ...
                                alt="author of blog"
                              />
                              <span className="text-[16px] text-[#F5F5F5] font-normal leading-normal truncate">
                                Author: {blog.author}
                              </span>
                            </div>
        ...
        {isLoading ? (
          ...
          <Carousel
            opts={{ align: "start" }}
            className="w-full flex justify-center items-center max-w-[900px] xl:max-w-[1000px] bg-[#171717]"
          >
            <CarouselContent className="-ml-1 bg-[#171717]">
              {popularBlogs.slice(0, 9).map((blog: any, index: number) => (
                <CarouselItem
                  key={blog._id}
                  className={`md:basis-auto bg-[#171717] ${index === 0 ? "pl-2" : "pl-[30px]"}`}
                >
                  <div
                    key={blog._id}
                    className="flex flex-col gap-4 w-[304px] md:w-[304px] h-auto relative "
                  >
                    <Link
                      href={`/blogs/${blog._id}`}
                      className="flex flex-wrap justify-center items-center h-auto gap-2 cursor-pointer"
                    >
                      <div className="flex flex-col gap-4 w-[304px] md:w-[304px] h-auto">
                        <div className="relative">
                          <Image
                        ...
                        <div className="w-[304px] flex flex-col gap-2">
                          ...
                          <div className="text-[12px] text-[#A3A3A3] font-normal leading-normal">
      ...
      <Pagination
        className={`${filteredBlogs.length <= 9 ? "hidden" : "flex"} text-[#A3A3A3] text-[24px] font-medium leading-[35px] mb-10`}
      >
        <PaginationContent>
          <PaginationItem>
            <PaginationPrevious
...
export default DesktopFilterForBlog;
