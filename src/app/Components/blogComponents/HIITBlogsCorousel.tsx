...
const CardioBlogsCorousel = () => {
  ...
  const [yogaBlogs, setYogaBlogs] = useState<Blog[]>([]);
  ...
  const getBlogsByCategory = (blogs: Blog[], category: string) => {
    const yogaCategoryBlogs = blogs.filter(
      (blog) => blog.category.toLowerCase() === category.toLowerCase(),
    );
    //console.log("yogaCategoryBlogs", yogaCategoryBlogs);
    setYogaBlogs(yogaCategoryBlogs);
  ...
  useEffect(() => {
    getBlogsByCategory(blogs, "Yoga");
  }, [blogs]);

  return (
    <>
      {/* <div className="block mt-4 embla md:hidden mb-7" ref={emblaRef}>
        <div className="embla__container">
          {yogaBlogs.slice(0, 3).map((item) => (
            <Link
              href={`/blogs/${item._id}`}
              key={item._id}
            ...
            >
              ...
              <div className="flex items-center justify-between mt-2">
                <div className="flex items-center justify-center gap-2">
                  <Image
                    src={item.profileImage ? item.profileImage : AuthorPic}
