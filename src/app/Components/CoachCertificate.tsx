...
const CoachCertificate = ({ data, id }: any) => {
  const apiEndpoint =  process.env.NEXT_PUBLIC_BASE_API_ENDPOINT;
  const [date, setDate] = React.useState<Date | undefined>(new Date());
  const [coachDetails, setCoachDetails] = useState<Certificate[]>();
  ...
  const fetchCoachCertificate = async () => {
    if (data) {
      try {
        ...
        if (response.data.success) {
          setCoachDetails(response.data.certificate);
        }
      } catch (error) {
      ...
      <section className="flex justify-center items-start w-full px-4 md:px-[72px] gap-[32px]">
        <div className="flex flex-col items-center justify-center flex-1">
          <div className="about-coach-sub-div w-[328px] md:w-full h-auto md:h-auto py-6 md:py-10 px-6 md:px-[49px] flex flex-col gap-4 md:gap-[30px]">
            ...
            {coachDetails &&
              coachDetails.map((certificate: any) => (
                <div className="flex gap-6" key={certificate.name}>
                  <div className="pt-4">
                    <svg
                    ...
                    >
                      <circle
                        ...
                        cy="10"
                        r="10"
                        fill="url(#paint0_linear_4001_30168)"
                      />
                      <circle cx="10" cy="10" r="3" fill="white" />
                      <defs>
                        <linearGradient
                          id="paint0_linear_4001_30168"
                          x1="-0.769231"
                          y1="12.766"
                          x2="20.6731"
                          y2="12.7694"
                  ...
                  <div>
                    <h2 className="text-[#D4D4D4] text-[20px] md:text-[32px] font-bold leading-[30px] md:leading-[49px]">
                      {certificate.name}
                    </h2>
                    <h5 className="text-[#A3A3A3] text-[18px] md:text-[24px] font-bold leading-[30px] md:leading-[49px]">
                      {certificate.provider}
                    </h5>
                    <p className="text-[#737373] text-[16px] md:text-[20px] font-medium leading-[30px] md:leading-[49px]">
                      Issued on {formatDate(certificate.validityFrom)}
                    </p>
                  </div>
                </div>
              ))}
