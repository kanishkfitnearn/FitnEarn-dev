...
function SignupForm() {
  ...
  const inviteToken = searchParams.get('invite_token'); // Grab the invite_token from query params
  // //console.log("invite_token:", inviteToken);
  if(inviteToken){
    Cookie.set('invite_token', inviteToken, { expires: 1 });
  }

  return (
    <>
      <div className="flex justify-center w-full h-screen bg-black mobBackImg">
        <div className="flex-1 hidden bg-center bg-cover leftImg md:block"></div>
        <div className="flex flex-col flex-1/2 ">
          ...
          <div className="middleMidImg min-h-screen md:min-h-0 md:flex-[2] flex justify-center items-center bg-cover bg-center">
            <form
              className="card signup-card flex justify-center items-center flex-col max-w-md w-[328px] h-[320px] md:w-[412px] md:h-[370px] space-y-8 shadow rounded-[24px] bg-gray-900 bg-opacity-60 backdrop-blur-xl"
              action="/api/auth/google-sign-up"
              method="GET"
            >
        ...
        <div className="flex-1 hidden bg-center bg-cover rightImg md:block"></div>
      </div>
    </>
  );
}
