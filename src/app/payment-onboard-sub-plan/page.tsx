...
const Page = () => {
  ...
  //for planid 4 price -> 7200; discount -> 20 %; finalPrice -> 5760
  //for planid 3 price -> 700; discount -> 25 %; final Price -> 525
  //for planid 2 price -> 600; discount -> 12 %; final Price -> 528
  //for planid 1 price -> 0; discount -> 10 %; final Price -> 0
  ...
  useEffect(() => {
    const planId = parseInt(Cookies.get("planId") || "0", 10);

    let price = 0;
    let discountPercentage = 0;

    // Logic for price and discount
    switch (planId) {
      case 4:
        price = 7200;
    ...
    setPlanDetails({ price, discount, finalPrice });
  }, []);
  ...
  const createSubscription = async () => {
    ...
    try {
      ...
      Cookies.set("planId", planId.toString());
    } catch (error) {
      //console.log(error);
    }
  };
  ...
  const planId = parseInt(Cookies.get("planId") || "");
  ...
  useEffect(() => {
    ...
    const isYearly = planId > 2;
    setPlanType(isYearly ? "Yearly" : "Monthly");

    // Calculate validity date
    const today = new Date();
    const expiryDate = new Date(today);
    expiryDate.setDate(today.getDate() + (isYearly ? 365 : 30));

    // Format the date (e.g., MM/DD/YYYY or DD/MM/YYYY based on locale)
    const formattedDate = expiryDate.toLocaleDateString(); // Adjust locale if needed
  ...
  const finalPriceString: number = planDetails.finalPrice;

  const amount = finalPriceString * 100;
  ...
  const paymentHandler = async (e: any) => {
    ...
    if (amount === 0) {
      console.log("Bypassing payment screen for zero amount.");
      // Mock Razorpay response for zero amount
      const mockResponse = {
        razorpay_payment_id: `mock_payment_${generateRandomId()}`,
        razorpay_order_id: `mock_order_${generateRandomId()}`,
      };
  
      try {
        await handlePaymentMode(payMode, mockResponse);
        setLoading(false);
      } catch (error) {
        console.error("Error in post-payment processing for zero amount:", error);
    ...
    if (!selectedValue) {
      ...
      try {
        ...
        toast({
          ...
          description: "You Have signed up to Fitnearn .",
          duration: 5000,
        });
