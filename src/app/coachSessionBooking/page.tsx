...
const CoachSessionBookingComponent = () => {
  ...
  const TimeSlotComponent = () => (
    <div className="text-white">
      <div className="flex flex-wrap justify-center items-center gap-4 py-3.5">
  ...
  const CalendarComponent = () => (
    <div className="">
      <div className=" w-[285px] h-auto flex flex-col justify-center items-center pb-2 px-2 pt-2">
        <Calendar
          mode="single"
          ...
          onSelect={(day) => {
            if (day && isDateAllowed(day)) {
              // setCalenderDate(day);
              CheckSessionConflictsForDate(day);
            }
          }}
  ...
  const handleBooking = () => {
    // const url = `/inviteForSession?coachId=${coachDetails?._id}&sessionId=${metadata.sessionId}`;
    const url = `/inviteForSession?coachId=${coachDetails?._id}`;
    push(url);
  };
  ...
  const fetchCoachDetails = async () => {
    try {
      const response = await axios.get(
        `${apiEndpoint}/api/fitnearn/web/coach/profile/${Id}`,
      );
      //console.log("fetchCoachDetails", response.data);
  ...
  const CheckSessionConflictsForDate = async (date: Date) => {
    if (userName && !genToken) {
      router.push("/login");
      return;
    }
    ...
    try {
      const response = await fetch(
        `${apiEndpoint}/api/fitnearn/web/coach/conflict/${Id}?date=${formattedDate}&timeSlot=${selectedTime ? selectedTime : timeSlot}`,
      );
      const result = await response.json();
      if (result.success) {
        // const url = `/coachSessionBooking?Id=${id}`;
  ...
  // invite friends related functions
  useEffect(() => {
  ...
  const finalPrize =
    sessionPrize && sessionDiscount
      ? ((sessionPrize * (1 - sessionDiscount / 100)) as number)
      : sessionPrize;
  ...
  //     payMode = "wallet";
  //   }
  //   if(selectedValue === "Credit & Debit Cards"){
  //     payMode = "card";
  //   }
  ...
  //       currency,
  //       receipt: receiptId,
  //       method: payMode, // wallet, netbanking, upi, card,
  //     }),
  //     headers: {
  ...
  // const handlePrivateSessionPayment = async () => {
  //   if (stopBooking) {
  //     toast({
  //       title: "select a right timeSlot or date.",
  //       description:
  //         "Please choose any other time slot or date for booking a session.",
  //     });
  //     return;
  //   }
  //   console.log("requestedInvitees", requestedInvitees);
  //   console.log("invitees from handlePrivateSessionPayment", invitees);
  //   console.log("Id of coach from handlePrivateSessionPayment", Id);
  //   console.log("Id of coachId from handlePrivateSessionPayment", coachId);
  //   const discountedPrice = sessionPrize && sessionDiscount ?  sessionPrize * (1 - sessionDiscount / 100) : sessionPrize;
  //   const sessionDetails = {
  //     title: `${coachCategory} Session`,
  //     coachId: Id,
  //     date: formattedDate,
  //     timeSlot: timeSlot,
  //     userId: userId,
  //     category: coachCategory,
  //     participantsEmail: Invitees.length > 0 ? Invitees : invitees,
  //     invitedEmail: requestedInvitees,
  //     seats: seatCounter,
  //     price: sessionPrize,
  //   };
  ...
  //   try {
  //     const res = await fetch(
  //       `${apiEndpoint}/api/fitnearn/web/users/session/private/book`,
  //       {
  //         method: "POST",
  //         headers: {
  //           "Content-Type": "application/json",
  ...
  //     const result = await res.json();
  //     console.log("Private session successfully created:", result);
  //     setPrivateSessionId(result.privateSession._id);
  //     recordPaymentDetails("Private", result.privateSession._id);
  //   } catch (error) {
  //     console.error("Error in creating private session:", error);
  //   }
  // };
  ...
  //   session_id: string,
  // ): Promise<void> => {
  //   console.log("Flag for recordPaymentDetails:", flag);
  //   console.log("sessionId:", session_id);
  //   console.log("privateSessionId:", privateSessionId);

  //   const randomTransactionId = Math.floor(1000 + Math.random() * 9000); // Generate a random 4-digit number
  ...
  //     // Log response details to inspect it more closely
  //     // console.log("Response status:", response.status);

  //     // Read the response once as JSON
  //     const result = await response.json(); // Read as JSON
  ...
  //           description:
  //             "Your session has been successfully booked. Enjoy the session!",
  //         });
  //         setTimeout(() => {
  //           router.push(
  ...
  const handlePrivateSessionPayment = async (
    ...
    try {
      ...
      console.log("Private session successfully created:", result);
      const sessionId =
        result.privateSession && result.privateSession._id
          ? result.privateSession._id
          : "";
      if (sessionId) {
        console.log("Recording payment details for session:", sessionId);
        ...
        console.error("Cannot record payment details: session ID is missing.");
      }
      // setPrivateSessionId(result.privateSession._id);
      // recordPaymentDetails("Private", result.privateSession._id);
    } catch (error) {
  ...
  return (
    <div className="pt-[72px] md:pt-[92px] mb-[70px]">
            ...
            >
              ...
              <div>
                {/* <Dialog>
                  ...
                  <DialogContent className="coach-dropdown mq450:w-[320px] sm:max-w-[320px] flex flex-col justify-center items-center pb-1">
                    <DialogTitle className="flex mid-heading text-[22px]">
                      Select Date
                    </DialogTitle>
                    <div className=" w-[285px] h-auto flex flex-col justify-center items-center pb-2 px-2">
                      <Calendar
                        mode="single"
                        selected={calenderDate}
                        onSelect={(day) => {
                ...
                <Dialog open={isOpen} onOpenChange={setIsOpen}>
                  ...
                  <DialogContent className="coach-dropdown h-[518px] mq450:w-[320px] sm:max-w-[320px] flex flex-col justify-center items-center pb-1">
                    <DialogTitle className="flex mid-heading text-[22px]">
                      Select Date and Time
                    </DialogTitle>
                    <div className="w-[320px] h-auto flex flex-col justify-center items-center gap-4">
                      <div className="flex justify-between ">
                        <button
                          onClick={() => toggleView("date")}
                          className={`${isDateActive ? "dateAndTimeToggleBtn rounded-tl-[8px] rounded-bl-[8px] rounded-tr-none rounded-br-none" : "bg-[#171717] border-[1px] border-[#F43F5E]"}
                          px-4 py-2 text-[16px] text-white font-medium rounded-tl-[8px] rounded-bl-[8px] rounded-tr-none rounded-br-none`}
                        >
                          Date
                        </button>
                        <button
                          onClick={() => toggleView("time")}
                          className={`${!isDateActive ? "dateAndTimeToggleBtn rounded-tr-[8px] rounded-br-[8px] rounded-tl-none rounded-bl-none" : "bg-[#171717] border-[1px] border-[#F43F5E]"} 
                          px-4 py-2 text-[16px] text-white font-medium rounded-tr-[8px] rounded-br-[8px] rounded-tl-none rounded-bl-none `}
                        >
                          Time
                        </button>
                      </div>
              ...
              <div>
                <Dialog open={isInviteOpen} onOpenChange={setIsInviteOpen}>
                  ...
                  <DialogContent className="bg-[#262626] mq450:w-[328px] mq450:pt-0 sm:max-w-[1024px] rounded-[12px] flex flex-col justify-center items-center pb-1">
                    <DialogTitle>
                      <div className="flex items-center gap-1 md:gap-4 mq450:mt-4 md:mb-5">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                        ...
                        <span className="text-[20px] md:text-[32px] text-[#FFFFFF] font-bold leading-normal">
                          Lets Invite your Friend
                        </span>
                      </div>
                    </DialogTitle>
                    <form onSubmit={handleInvite}>
                      <div className="flex md:flex-row flex-col justify-center items-start gap-[20px]">
                        <section className="flex">
                          <div className=" h-full w-[328px] md:w-[1024px] px-5 md:px-10  pb-6 md:pb-6 pt-0">
                            ...
                            <div className="items-start justify-between hidden w-full gap-0 mt-4 md:flex">
                              <div className=" h-full text-[14px] text-[#A3A3A3] font-normal leading-normal pb-3">
                                {invitees && invitees.length > 0 ? (
                                  ...
                                  <span>
                                    You Have select only {seatCounter} seat so
                                    you can also add only as per your
                                    requirement
                                  </span>
                                )}
                              </div>
                      ...
                      <div className="flex flex-col items-center justify-center gap-3 mb-4 md:mb-8 md:gap-6 ">
                        <DialogClose
        ...
        <div className="flex items-start justify-center flex-1 mt-5 mq450:p-4 md:mt-0">
          <div className="w-full mq450:max-w-[482px] md:w-[522px] about-coach-sub-div p-[22px] md:p-8">
            <div className="flex justify-start items-center gap-10 md:gap-[53px] pb-6">
              <Image
                src={coachDetails ? coachDetails.profileImage : CoachProfile}
                alt="Coach Profile"
                width={120}
                height={120}
              ...
              <div>
                ...
                <p className="text-[18px] md:text-[24px] text-[#FFFFFF] font-medium leading-normal">
                  {coachDetails?.categoryLevel} : {coachDetails?.category}
                </p>
              </div>
            </div>
            ...
            <div className="text-[#FFFFFF] pt-3">
              ...
              <p className="text-[16px] md:text-[20px] font-normal leading-normal">
                ...
                {formattedDate ? formatDateForCoachCard(formattedDate) : ""},{" "}
                {timeSlot ? timeSlot : ""} (IST).
              </p>
              {/* <Link
                href={"https://www.fitnearn.com/terms-conditions"}
              ...
              <button
                className="text-[18px] md:text-[20px] font-bold leading-normal cursor-pointer underline"
      ...
      <div className="flex justify-center items-center mt-[50px]">
        {isPayBtnOn ? (
          <Button
            // onClick={handlePrivateSessionPayment}
...
const CoachSessionBooking = () => (
  <Suspense fallback={<div>Loading...</div>}>
    <CoachSessionBookingComponent />
  </Suspense>
);

export default CoachSessionBooking;
