...
import newWorkoutHeaderImg from "../../../public/newWorkoutHeader.png";
import VideosSkeleton from "./videosSkeleton";
import axios from "axios";
import Cookies from "js-cookie";
import NotFoundSvg from "../Components/NotFoundSvg";
import DancingPng from "../../../public/dancingPng.png";
import newHeader from "../../../public/newerWorkoutHeader.png";
...
const Workout = () => {
  ...
  const [focusArea, setFocusAreas] = useState<string[]>([]);
  ...
  const [likedSessions, setLikedSessions] = useState<any[]>([]);
  ...
  //   (liveSession ? liveSessionData : combinedData).length / itemsPerPage,
  // );
  // const totalPages = 10;
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = (liveSession ? liveSessionData : combinedData).slice(
    startIndex,
    endIndex,
  );
  ...
  //   //console.log("Calling handlePageChange with page:", targetPage);
  //   fetchVideos(targetPage);
  //   setCurrentPage(targetPage);
  //   //console.log("Page updated to:", targetPage); // Should log after setCurrentPage
  //   // if (targetPage < totalPages) {
  //   //   //console.log("Preloading next page:", targetPage + 1);
  //   //   fetchVideos(targetPage); // Preload next page
  //   // }
  // };

  const handlePageChange = (targetPage: number) => {
    setCurrentPage(targetPage); // Update page number
    fetchVideos(targetPage); // Fetch data for the new page
  };
  ...
  // useEffect(() => {
  //   if (liveSession && liveSessionData.length <= 9) {
  //     setCurrentPage(1);
  //   }
  //   if (combinedData && combinedData.length <= 9) {
  //     setCurrentPage(1);
  //   }
  // }, [combinedData, liveSessionData]);
  ...
  const exerciseLevels = ["BEGINNER", "INTERMEDIATE", "ADVANCED"];
  const timeOption = [
  ...
  const intensities = ["HIGH", "LOW", "MODERATE"];
  const calories = [
  ...
  const getEquipments = async () => {
    try {
      const res = await fetch(
        `${apiEndpoint}/api/fitnearn/web/users/workout/equipment/get`,
      );
      const result = await res.json();
      //console.log("getEquipments", result);
  ...
  // const userId = "6668244b91ca7fc3a382d033";

  const fetchVideos = (page: number) => {
    setIsLoading(true);
    let internalFilter = [...categories];
    if (
      SingleCategory !== "" &&
      SingleCategory !== "All" &&
      !categories.includes(SingleCategory)
    ) {
      internalFilter.push(SingleCategory);
    }

    const encodedEquipment = selectedEquipment
      .map((item) => encodeURIComponent(item))
      .join(",");
    const query = `?page=${page}&internalFilter=${internalFilter.join(",")}&contentType=${content}&postedAt=${postedAt}&showLiveSessions=${liveSession}&searchQuery=${searchQuery}&calories=${calorie.join(",")}&duration=${time.join(",")}&intensityLevel=${intensity.join(",")}&equipment=${encodedEquipment}&focusArea=${focusArea.join(",")}&level=${exerciseLevel.join(",")}`;
    //console.log(query);

    axios
      .get(
        `${apiEndpoint}/api/fitnearn/web/users/workout-live-sessions${query}`,
        {
          headers: {
      ...
      .then((response) => {
        ...
        console.log("response data from fetch api", response.data.data);
        if (response.data.success) {
          if (liveSession) {
            //console.log("live session toggle on");
            // setLiveSessionData(response.data.data.liveSessions);
            setLiveSessionData((prevData) => [
              ...(page === 1 ? [] : prevData), // Clear previous data if loading the first page
              ...response.data.data.liveSessions,
            ]);
            if (response.data.data.liveSessions.length === 0) {
              setVideoData([]);
              setSessions([]);
              setCombinedData([]);
              setLiveSessionData([]);
            ...
            if (
              ...
              setCombinedData((prevData) => {
                const newData = page === 1 ? [] : [...prevData]; // Reset on first page
                response.data.data.videos.forEach((video: any) => {
                  if (!newData.some((item) => item.id === video.id)) {
                    newData.push(video);
                  }
                });
                return newData;
  ...
  const fetchLikedSessions = async () => {
    ...
    try {
      const response = await fetch(
        // `http://localhost:9000/api/fitnearn/web/users/workout/live-session/get/likes/${userId}`,
        `${apiEndpoint}/api/fitnearn/web/users/workout/live-session/get/likes/${userId}`,
      );
      const result = await response.json();
      //console.log(result);
      if (result.success) {
        const likedSessionIds = result.data.map((session: any) => session._id);
        //console.log("likedSessionIds", likedSessionIds);
        // Update the likedSessions state with the extracted _id array
        setLikedSessions(likedSessionIds);
      } else {
        //console.log("something wrong in fetching liked videos");
      }
    } catch (error) {
  ...
  const manageOtherFilters = (categoryString: string, manageFor: string) => {
    if (manageFor === "equipments") {
      setSelectedEquipment((prevSelectedEquipments: string[]) => {
        if (prevSelectedEquipments.includes(categoryString)) {
          // Remove categoryString if already selected
          ...
          // Add categoryString if not already selected
          return [...prevSelectedEquipments, categoryString];
        }
      });
    }
    if (manageFor === "focusArea") {
      setFocusAreas((prevSelectedAreas: string[]) => {
        if (prevSelectedAreas.includes(categoryString)) {
          // Remove categoryString if already selected
          return prevSelectedAreas.filter((item) => item !== categoryString);
        } else {
          // Add categoryString if not already selected
          return [...prevSelectedAreas, categoryString];
        }
      });
    }
    if (manageFor === "exerciseLevel") {
      setExerciseLevel((prevSelectedLevel: string[]) => {
        if (prevSelectedLevel.includes(categoryString)) {
          // Remove categoryString if already selected
          return prevSelectedLevel.filter((item) => item !== categoryString);
        } else {
          // Add categoryString if not already selected
          return [...prevSelectedLevel, categoryString];
        }
      });
    }
    if (manageFor === "time") {
      setTime((prevSelectedTime: string[]) => {
        if (prevSelectedTime.includes(categoryString)) {
          // Remove categoryString if already selected
          return prevSelectedTime.filter((item) => item !== categoryString);
        } else {
          // Add categoryString if not already selected
          return [...prevSelectedTime, categoryString];
        }
      });
    }
    if (manageFor === "intensity") {
      setIntensity((prevSelectedIntensity: string[]) => {
        if (prevSelectedIntensity.includes(categoryString)) {
          // Remove categoryString if already selected
          return prevSelectedIntensity.filter(
            (item) => item !== categoryString,
          );
        } else {
          // Add categoryString if not already selected
          return [...prevSelectedIntensity, categoryString];
        }
      });
    }
    if (manageFor === "calorie") {
      setCalorie((prevSelectedCalorie: string[]) => {
        if (prevSelectedCalorie.includes(categoryString)) {
          // Remove categoryString if already selected
          return prevSelectedCalorie.filter((item) => item !== categoryString);
        } else {
          // Add categoryString if not already selected
          return [...prevSelectedCalorie, categoryString];
        }
      });
    }
  };
  ...
  useEffect(() => {
    setCurrentPage(1);
    fetchVideos(1);
    //console.log("single category selected:", SingleCategory);
    //console.log("categories selected:", categories);
    //console.log("Content selected:", content);
    //console.log("posted at:", postedAt);
    //console.log("live session toggle:", liveSession);
  }, [SingleCategory, liveSession]);

  // const combination = () => {
  // //console.log("video data in combination", videoData);
  // //console.log("session data in combination", sessions);
  // combinedDataArray = [...videoData, ...sessions];
  // //console.log("combinedData", combinedDataArray);
  // for (let i = combinedDataArray.length - 1; i > 0; i--) {
  //   const j = Math.floor(Math.random() * (i + 1));
  //   [combinedDataArray[i], combinedDataArray[j]] = [
  //     combinedDataArray[j],
  //     combinedDataArray[i],
  //   ];
  // }
  ...
  // useEffect(() => {
  //   combination();
  //   //console.log("combinedData after combination", combinedData);
  // }, [videoData]);
  ...
  function formatDate(dateString: string) {
    ...
    const year = String(date.getUTCFullYear()).slice(-4); // Slice last 4 digits of year
  ...
  function getTimeDifference(startTime: string, endTime: string): string {
    ...
    const diffMin: number = Math.floor(diffMs / (1000 * 60));
    const hours: number = Math.floor(diffMin / 60);
    const minutes: number = diffMin % 60;
    return `${hours} Hours ${minutes} min`;
  }

  // const upcomingOrLiveSessions = liveSessionData.filter((item: any) => {
  //   const sessionDateTime = new Date(item.dateTime);
  ...
  return (
    <div className="pt-[72px] md:pt-[72px]">
      <div>
        {/* this is for mobile only it contains search, filter, corousel and toggle btn */}
        <div className="flex flex-col items-center justify-center px-4 md:hidden">
                    ...
                    <div className="flex w-full flex-col justify-start items-start gap-4 mb-[18px] max-h-[60vh] overflow-y-auto py-3">
                      <div className="flex w-full flex-col justify-start items-start gap-4 pb-4 border-b-[1px] border-[#a3a3a352]">
                        ...
                        <div className="flex items-center justify-start w-full gap-2">
                          <div
                            onClick={() => setContent("Free")}
                            className={`${content === "Free" ? "fitness-level-selected" : ""} py-2 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                          >
                            FREE
                          </div>
                          <div
                            onClick={() => setContent("Premium")}
                            className={`${content === "Premium" ? "fitness-level-selected" : ""} py-2 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                          >
                            PREMIUM
                          </div>
                        </div>
                      ...
                      <div className="flex w-full flex-col justify-start items-start gap-4 pb-4 border-b-[1px] border-[#a3a3a352]">
                        ...
                        <div className="flex items-center justify-start w-full gap-2">
                          <div
                            onClick={() => setPostedAt("TODAY")}
                            className={`${postedAt === "TODAY" ? "fitness-level-selected font-medium" : ""} py-2 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                          >
                            TODAY
                          </div>
                          <div
                            onClick={() => setPostedAt("WEEK")}
                            className={`${postedAt === "WEEK" ? "fitness-level-selected font-medium" : ""} py-2 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                          >
                            WEEK
                          </div>
                          <div
                            onClick={() => setPostedAt("MONTH")}
                            className={`${postedAt === "MONTH" ? "fitness-level-selected font-medium" : ""} py-2 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                          >
                            MONTH
                          </div>
                        </div>
                          ...
                          {filterCategories.map((item) => {
                            ...
                            return (
                              <div
                                ...
                                className={`${
                                  categories.includes(formattedItem) ||
                                  SingleCategory === formattedItem
                              ...
                              <div className="flex flex-wrap items-center justify-start w-full gap-2 cursor-pointer">
                                {equipments.map((item) => (
                                  <div
                                    ...
                                    className={`${selectedEquipment.includes(item.equipment) ? "fitness-level-selected font-semibold" : ""}
                                inline-block py-2 px-4 bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal`}
                                  >
                                    {item.equipment}
                                  </div>
                              ...
                              <div className="flex flex-wrap items-center justify-start w-full gap-2 cursor-pointer">
                                {focusAreas.map((item) => (
                                  <div
                                    key={item}
                                    onClick={() =>
                                      manageOtherFilters(item, "focusArea")
                                    }
                                    className={`${focusArea.includes(item) ? "fitness-level-selected font-semibold" : ""}
                                inline-block py-2 px-4 bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal`}
                                  >
                                    {item}
                                  </div>
                              ...
                              <div className="flex flex-wrap items-center justify-start w-full gap-2 cursor-pointer">
                                {exerciseLevels.map((item) => (
                                  <div
                                    key={item}
                                    onClick={() =>
                                      manageOtherFilters(item, "exerciseLevel")
                                    }
                                    className={`${exerciseLevel.includes(item) ? "fitness-level-selected font-semibold" : ""}
                                inline-block py-2 px-4 bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal`}
                                  >
                                    {item}
                                  </div>
                              ...
                              <div className="flex flex-wrap items-center justify-start w-full gap-2 cursor-pointer">
                                {timeOption.map((item) => (
                                  <div
                                    key={item.name}
                                    onClick={() =>
                                      manageOtherFilters(item.value, "time")
                                    }
                                    className={`${time.includes(item.value) ? "fitness-level-selected font-semibold" : ""}
                                inline-block py-2 px-4 bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal`}
                                  >
                                    {item.name}
                                  </div>
                              ...
                              <div className="flex flex-wrap items-center justify-start w-full gap-2 cursor-pointer">
                                {intensities.map((item) => (
                                  <div
                                    key={item}
                                    onClick={() =>
                                      manageOtherFilters(item, "intensity")
                                    }
                                    className={`${intensity.includes(item) ? "fitness-level-selected font-semibold" : ""}
                                inline-block py-2 px-4 bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal`}
                                  >
                                    {item}
                                  </div>
                              ...
                              <div className="flex flex-wrap items-center justify-start w-full gap-2 cursor-pointer">
                                {calories.map((item) => (
                                  <div
                                    key={item.name}
                                    onClick={() =>
                                      manageOtherFilters(item.value, "calorie")
                                    }
                                    className={`${calorie.includes(item.value) ? "fitness-level-selected font-semibold" : ""}
                                inline-block py-2 px-4 bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[14px] text-[#DADADA] font-normal leading-normal`}
                                  >
                                    {item.name}
                                  </div>
          ...
          <div className="flex w-[326px] h-[46px] justify-center items-center gap-3 border-[1px] border-solid border-[#FFF] rounded-[8px] mt-3">
        ...
        <div className="hidden md:flex relative w-full h-[522px] justify-center items-center">
          ...
          <div className="absolute flex flex-col justify-center items-center w-full max-w-[1140px]">
            ...
            <div className="flex justify-center items-center mt-[52px]">
              <div className="relative w-[481px] h-[50px]">
                <input
                  type="search"
                  id="location-search"
                  className="block w-full h-[50px] py-[2px] px-[2.5px] z-20 text-[16px] font-medium leading-[24px] pl-4 text-[#A3A3A3] bg-[#171717] rounded-e-[10px] border-s-[#A3A3A3] border-s-2 border border-[#A3A3A3] focus:ring-[#FFFFFF] focus:border-none dark:bg-gray-700 dark:border-s-gray-700  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-[#FFFFFF]"
                  placeholder="Search your workout, live Session"
                ...
                <button
                  type="submit"
                  className="absolute h-[50px] w-[43px]  top-0 end-0 p-2.5 text-sm font-medium text-white bg-blue-700 rounded-e-lg border border-blue-700 hover:bg-blue-800 focus:ring-1 focus:outline-none focus:ring-[#FFFFFF] dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-[#FFFFFF]"
                  style={{
                    borderRadius: "0px 8px 8px 0px",
                    border: "1px solid var(--Orange-500-main, #F97316)",
                    background:
                      "var(--Linears-linear-main, linear-gradient(90deg, #F43F5E -3.84%, #FB923C 103.36%))",
                  }}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
        ...
        {/* this is for desktop it contains search, filter, corousel and toggle btn */}
        <div className="hidden md:h-[91px] bg-[#171717] z-50 sticky top-[70px] md:flex w-full justify-center items-center pl-[120px] pr-[60px] border-y-[1.5px] border-y-[#262626] py-4">
          <div className="w-[1200px] max-w-[1200px] flex justify-between items-center">
                ...
                <CarouselItem className="md:basis-auto bg-[#171717] pl-[24px]">
                  <div className="bg-[#171717] flex justify-center items-center relative">
                    ...
                    >
                      ...
                      {/* {SingleCategory === "All" && (
                      <div className="absolute bottom-0 left-0 right-0 h-[4px] bg-gradient-to-r from-[#F43F5E] to-[#FB923C] mt-1" />
                    )} */}
                    </button>
                  </div>
                </CarouselItem>
                ...
                <CarouselItem className="md:basis-auto bg-[#171717] pl-[32px]">
                  <div className="bg-[#171717] relative">
                    ...
                    >
                      ...
                      {/* {SingleCategory === "Strength & conditioning" && (
                      <div className="absolute bottom-0 left-0 right-0 h-[4px] bg-gradient-to-r from-[#F43F5E] to-[#FB923C] mt-1" />
                    )} */}
                    </button>
                  </div>
                ...
                <CarouselItem className="md:basis-auto bg-[#171717] pl-[32px]">
                  <div className="bg-[#171717] relative">
                    ...
                    >
                      ...
                      {/* {SingleCategory === "General workout" && (
                      <div className="absolute bottom-0 left-0 right-0 h-[4px] bg-gradient-to-r from-[#F43F5E] to-[#FB923C] mt-1" />
                    )} */}
                    </button>
                  </div>
            ...
            <div className="flex gap-7">
                      ...
                      <div className="flex w-full flex-col justify-start items-start py-6 gap-[18px] max-h-[450px] overflow-y-auto">
                        <div className="flex flex-col items-start justify-start w-full gap-4">
                          ...
                          <div className="flex items-center justify-start w-full gap-2 pb-6 border-b-[1px] border-[#a3a3a352]">
                            <div
                              onClick={() => setContent("Free")}
                              className={`${content === "Free" ? "fitness-level-selected font-semibold" : "bg-[#262626]"} py-3 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[16px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                            >
                              FREE
                            </div>
                            <div
                              onClick={() => setContent("Premium")}
                              className={`${content === "Premium" ? "fitness-level-selected font-semibold" : "bg-[#262626]"} py-3 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[16px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                            >
                              PREMIUM
                            </div>
                          </div>
                        ...
                        <div className="flex flex-col items-start justify-start w-full gap-4">
                          ...
                          <div className="flex items-center justify-start w-full gap-2 pb-6 border-b-[1px] border-[#a3a3a352]">
                            <div
                              onClick={() => setPostedAt("TODAY")}
                              className={`${postedAt === "TODAY" ? "fitness-level-selected font-semibold" : ""} py-3 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[16px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                            >
                              TODAY
                            </div>
                            <div
                              onClick={() => setPostedAt("WEEK")}
                              className={`${postedAt === "WEEK" ? "fitness-level-selected font-semibold" : ""} py-3 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[16px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                            >
                              THIS WEEK
                            </div>
                            <div
                              onClick={() => setPostedAt("MONTH")}
                              className={`${postedAt === "MONTH" ? "fitness-level-selected font-semibold" : ""} py-3 px-4 inline-block bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[16px] text-[#DADADA] font-normal leading-normal cursor-pointer`}
                            >
                              THIS MONTH
                            </div>
                          </div>
                        ...
                        <div className="flex w-full flex-col justify-start items-start pb-6 gap-4 border-b-[1px] border-[#a3a3a352]">
                          <h2 className="text-[24px] text-[#E5E5E5] font-bold leading-[36px]">
                            Categories
                          </h2>
                          <div className="flex flex-wrap items-center justify-start w-full gap-2 cursor-pointer">
                                ...
                                <div className="flex flex-wrap items-center justify-start w-full gap-2 cursor-pointer">
                                  {equipments.map((item) => (
                                    <div
                                      ...
                                      className={`${selectedEquipment.includes(item.equipment) ? "fitness-level-selected font-semibold" : ""}
                                inline-block mb-1 px-4 py-3 bg-[#262626] border-[1px] border-[#262626] rounded-[8px] text-[16px] text-[#DADADA] font-normal leading-normal uppercase`}
                                    >
                                      {item.equipment}
                                    </div>
                                  ))}
              ...
              <div className="flex w-[180px] h-[46px] justify-center items-center gap-3 border-[1px] border-solid border-[#FFF] rounded-[8px]">
        ...
        <div
          className={`${(videoData.length <= 0 && sessions.length <= 0) || (liveSession === true && liveSessionData.length <= 0) ? "md:pt-0 md:pb-10" : "md:py-[50px]"} flex flex-col justify-center items-center py-6 px-4 md:px-[67px] `}
        >
          {isLoading ? (
            <VideosSkeleton />
          ) : (
            <div className="flex flex-wrap justify-center items-start h-auto gap-[48px] pb-[64px] ">
              {liveSession && liveSessionData && liveSessionData.length > 0
                ? currentItems.map((item: any) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        // className="flex flex-col gap-4 w-[328px] md:w-[350px] h-auto"
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="relative">
                            ...
                            />
                            {isLive && (
                              <span className="absolute bg-[#DC2626] rounded-[4px] text-[16px] text-[#FFFFFF] font-bold leading-normal py-[2px] px-2 left-[16px] top-[16px]">
                                Live
                              </span>
                            )}
                            {/* {likedSessions ? (
                          <LikeButton
                            videoId={item.id}
                            likeFor={"liveSession"}
                            // initialLiked={likedSessions.includes(item.id)}
                          ...
                          <div>
                            ...
                            <div className="flex items-center justify-between py-2">
                              <div className="flex items-center justify-center gap-1">
                                ...
                                >
                                  <path
                                    d="M12.9512 3.2H11.737V2.6C11.737 2.44087 11.6731 2.28826 11.5592 2.17574C11.4454 2.06321 11.291 2 11.1299 2C10.9689 2 10.8145 2.06321 10.7007 2.17574C10.5868 2.28826 10.5228 2.44087 10.5228 2.6V3.2H8.70154V2.6C8.70154 2.44087 8.63758 2.28826 8.52373 2.17574C8.40987 2.06321 8.25546 2 8.09444 2C7.93343 2 7.77901 2.06321 7.66516 2.17574C7.5513 2.28826 7.48734 2.44087 7.48734 2.6V3.2H5.66604V2.6C5.66604 2.44087 5.60208 2.28826 5.48822 2.17574C5.37437 2.06321 5.21995 2 5.05894 2C4.89793 2 4.74351 2.06321 4.62966 2.17574C4.5158 2.28826 4.45184 2.44087 4.45184 2.6V3.2H3.23764C2.91561 3.2 2.60678 3.32643 2.37907 3.55147C2.15136 3.77652 2.02344 4.08174 2.02344 4.4V12.8C2.02344 13.1183 2.15136 13.4235 2.37907 13.6485C2.60678 13.8736 2.91561 14 3.23764 14H12.9512C13.2733 14 13.5821 13.8736 13.8098 13.6485C14.0375 13.4235 14.1654 13.1183 14.1654 12.8V4.4C14.1654 4.08174 14.0375 3.77652 13.8098 3.55147C13.5821 3.32643 13.2733 3.2 12.9512 3.2ZM4.45184 4.4C4.45184 4.55913 4.5158 4.71174 4.62966 4.82426C4.74351 4.93679 4.89793 5 5.05894 5C5.21995 5 5.37437 4.93679 5.48822 4.82426C5.60208 4.71174 5.66604 4.55913 5.66604 4.4H7.48734C7.48734 4.55913 7.5513 4.71174 7.66516 4.82426C7.77901 4.93679 7.93343 5 8.09444 5C8.25546 5 8.40987 4.93679 8.52373 4.82426C8.63758 4.71174 8.70154 4.55913 8.70154 4.4H10.5228C10.5228 4.55913 10.5868 4.71174 10.7007 4.82426C10.8145 4.93679 10.9689 5 11.1299 5C11.291 5 11.4454 4.93679 11.5592 4.82426C11.6731 4.71174 11.737 4.55913 11.737 4.4H12.9512V5.6H3.23764V4.4H4.45184ZM3.23764 12.8V6.8H12.9512V12.8H3.23764Z"
                                    fill="url(#paint0_linear_2772_19006)"
                                  />
                                  <path
                                    d="M5.36249 8H4.75539C4.58774 8 4.45184 8.13432 4.45184 8.3V8.9C4.45184 9.06569 4.58774 9.2 4.75539 9.2H5.36249C5.53014 9.2 5.66604 9.06569 5.66604 8.9V8.3C5.66604 8.13432 5.53014 8 5.36249 8Z"
                                    fill="url(#paint1_linear_2772_19006)"
                                  />
                                  <path
                                    d="M5.36249 10.4H4.75539C4.58774 10.4 4.45184 10.5343 4.45184 10.7V11.3C4.45184 11.4657 4.58774 11.6 4.75539 11.6H5.36249C5.53014 11.6 5.66604 11.4657 5.66604 11.3V10.7C5.66604 10.5343 5.53014 10.4 5.36249 10.4Z"
                                    fill="url(#paint2_linear_2772_19006)"
                                  />
                                  <path
                                    d="M8.39799 8H7.79089C7.62325 8 7.48734 8.13432 7.48734 8.3V8.9C7.48734 9.06569 7.62325 9.2 7.79089 9.2H8.39799C8.56564 9.2 8.70154 9.06569 8.70154 8.9V8.3C8.70154 8.13432 8.56564 8 8.39799 8Z"
                                    fill="url(#paint3_linear_2772_19006)"
                                  />
                                  <path
                                    d="M8.39799 10.4H7.79089C7.62325 10.4 7.48734 10.5343 7.48734 10.7V11.3C7.48734 11.4657 7.62325 11.6 7.79089 11.6H8.39799C8.56564 11.6 8.70154 11.4657 8.70154 11.3V10.7C8.70154 10.5343 8.56564 10.4 8.39799 10.4Z"
                                    fill="url(#paint4_linear_2772_19006)"
                                  />
                                  <path
                                    d="M11.4335 8H10.8264C10.6587 8 10.5228 8.13432 10.5228 8.3V8.9C10.5228 9.06569 10.6587 9.2 10.8264 9.2H11.4335C11.6011 9.2 11.737 9.06569 11.737 8.9V8.3C11.737 8.13432 11.6011 8 11.4335 8Z"
                                    fill="url(#paint5_linear_2772_19006)"
                                  />
                                  <path
                                    d="M11.4335 10.4H10.8264C10.6587 10.4 10.5228 10.5343 10.5228 10.7V11.3C10.5228 11.4657 10.6587 11.6 10.8264 11.6H11.4335C11.6011 11.6 11.737 11.4657 11.737 11.3V10.7C11.737 10.5343 11.6011 10.4 11.4335 10.4Z"
                                    fill="url(#paint6_linear_2772_19006)"
                                  />
                                  <defs>
                                    <linearGradient
                                      id="paint0_linear_2772_19006"
                                      x1="1.55644"
                                      y1="9.65958"
                                      x2="14.5741"
                                      y2="9.66171"
                              ...
                              <div className="flex items-center justify-center gap-1">
                                ...
                                >
                                  <path
                                    d="M8.3608 14.6654C7.02665 14.6654 5.72247 14.2744 4.61317 13.5418C3.50386 12.8093 2.63927 11.7681 2.12871 10.5499C1.61816 9.33175 1.48457 7.99131 1.74485 6.6981C2.00513 5.40489 2.64758 4.21701 3.59097 3.28466C4.53435 2.35231 5.73629 1.71737 7.04481 1.46013C8.35332 1.2029 9.70962 1.33492 10.9422 1.8395C12.1748 2.34409 13.2283 3.19857 13.9695 4.2949C14.7107 5.39123 15.1064 6.68016 15.1064 7.9987C15.1044 9.76622 14.3931 11.4608 13.1285 12.7106C11.8638 13.9604 10.1492 14.6634 8.3608 14.6654ZM8.3608 2.66537C7.29348 2.66537 6.25013 2.97816 5.36269 3.5642C4.47525 4.15023 3.78357 4.98318 3.37513 5.95772C2.96669 6.93226 2.85982 8.00462 3.06804 9.03918C3.27626 10.0737 3.79023 11.0241 4.54493 11.7699C5.29964 12.5158 6.2612 13.0238 7.308 13.2296C8.35481 13.4353 9.43986 13.3297 10.4259 12.9261C11.412 12.5224 12.2548 11.8388 12.8478 10.9617C13.4408 10.0847 13.7572 9.05353 13.7572 7.9987C13.7556 6.5847 13.1866 5.22907 12.1749 4.22922C11.1632 3.22937 9.79153 2.66696 8.3608 2.66537Z"
                                    fill="url(#paint0_linear_2772_19009)"
                                  />
                                  <path
                                    d="M8.3608 8.66537C8.1819 8.66537 8.01032 8.59513 7.88382 8.47011C7.75731 8.34508 7.68624 8.17551 7.68624 7.9987V5.33203C7.68624 5.15522 7.75731 4.98565 7.88382 4.86063C8.01032 4.73561 8.1819 4.66537 8.3608 4.66537C8.5397 4.66537 8.71128 4.73561 8.83778 4.86063C8.96429 4.98565 9.03536 5.15522 9.03536 5.33203V7.9987C9.03536 8.17551 8.96429 8.34508 8.83778 8.47011C8.71128 8.59513 8.5397 8.66537 8.3608 8.66537Z"
                                    fill="url(#paint1_linear_2772_19009)"
                                  />
                                  <path
                                    d="M10.57 10.8487C10.3911 10.8487 10.2195 10.7784 10.0931 10.6534L7.88382 8.47011C7.76094 8.34437 7.69302 8.1759 7.69456 8.0011C7.69609 7.8263 7.76704 7.65909 7.89211 7.53549C8.01717 7.41188 8.18636 7.34177 8.36323 7.34025C8.54009 7.33873 8.71049 7.40593 8.83771 7.52737L11.0469 9.7107C11.1412 9.80394 11.2054 9.92271 11.2314 10.052C11.2574 10.1813 11.2441 10.3153 11.193 10.4371C11.142 10.5589 11.0556 10.663 10.9447 10.7363C10.8338 10.8096 10.7034 10.8487 10.57 10.8487Z"
                                    fill="url(#paint2_linear_2772_19009)"
                                  />
                                  <defs>
                                    <linearGradient
                                      id="paint0_linear_2772_19009"
                                      x1="1.09634"
                                      y1="9.84267"
                                      x2="15.5604"
                                      y2="9.84505"
                            ...
                            <div className="flex items-end justify-between">
                              <div className="flex flex-col gap-1">
                                <h3 className="text-[20px] text-[#FFFFFF] font-bold leading-normal">
                                  ...
                                  {item.seats - item.bookedSeats} Seats Left
                                </h3>
                                <div className="text-[16px] text-[#D4D4D4] font-semibold leading-normal flex justify-center items-center">
                                  <span className="text-[16px] text-[#F5F5F5] font-bold leading-normal mr-1">
                                    {/* {item.price ? `₹ ${(item.price * (1 - item.discountLiveSessionPercent / 100))}` : "₹ 250"} */}
                                    {userPlan === 3 || userPlan === 4
                                      ? `₹ ${item?.price * (1 - item?.discountLiveSessionPercent / 100)}`
                                      : `₹ ${item?.price}`}
                                  </span>
                                  <span className="text-[14px] text-[#A3A3A3] font-normal leading-normal line-through mr-2">
                                    {userPlan === 3 || userPlan === 4
                                      ? `₹ ${item?.price}`
                                      : ""}
                                  </span>
                                  {userPlan === 3 || userPlan === 4 ? (
                                    <span className="text-[12px] text-[#FACA15] font-medium leading-normal">
                                      {`${item.discountLiveSessionPercent}% discount applied`}
                                    </span>
                                  ) : (
                                    <Link
                                      href={"/subscription-plans"}
                                      className="text-[12px] text-[#FACA15] font-medium leading-normal"
                                    >
                                      {`Get (${item.discountLiveSessionPercent}% OFF)`}
                                    </Link>
                                  )}
                                </div>
                              </div>
                              <Link
                                ...
                                className="video-card-play-btn w-[120px] text-center text-[14px] text-[#FFFFFF] font-semibold leading-[21px] py-[10px] px-[20px]"
                              >
                                Book Now
                              </Link>
                            </div>
                ...
                : combinedData &&
                  currentItems.map((item: any) => {
                    ...
                    const endTime = new Date(item.endTime);
                    const currentTime = new Date();
                    // //console.log("session meetLink",item.meetLink);
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {/* card Upper part */}
                          {item.meetLink ? (
                            // if publishId that means it is a live session item
                            <div className="relative">
                              <Image
                                src={item.thumbnailUrl}
                                alt="video banner"
                              ...
                              />
                              {isLive && (
                                <span className="absolute bg-[#DC2626] rounded-[4px] text-[16px] text-[#FFFFFF] font-bold leading-normal py-1 px-2 left-[16px] top-[16px]">
                                  Live
                                </span>
                              )}
                              {/* {likedSessions ? (
              ...
              {videoData.length <= 0 && liveSessionData.length <= 0 ? (
                <NotFoundSvg />
              ) : liveSession === true && liveSessionData.length <= 0 ? (
                <NotFoundSvg />
              ) : (
                ""
              )}
          ...
          {/* <Pagination
            className={`${(videoData.length <= 0 && sessions.length <= 0) ||
              (liveSession === true && liveSessionData.length <= 0)
              ? "hidden"
              : "flex"
              } text-[#A3A3A3] text-[24px] font-medium leading-[35px]`}
          >
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious
          ...
          >
            <PaginationContent>
              ...
              {/* Ellipses for mobile and logic for visible pages */}
              {currentPage > (isMobile ? 3 : 5) &&
              ...
              {Array.from({ length: totalPages }, (_, index) => index + 1)
                ...
                .filter((page) => {
                  return (
                    totalPages <= (isMobile ? 5 : 7) || // Show all if total pages <= mobile/desktop limit
                    (page >= currentPage - (isMobile ? 0 : 1) &&
                      page <= currentPage + (isMobile ? 0 : 1)) // Show nearby pages based on device
                  );
                })
                .map((page) => (
                  <PaginationItem key={page}>
              ...
              {currentPage < totalPages - (isMobile ? 1 : 3) &&
                totalPages > (isMobile ? 5 : 7) && (
                  <PaginationItem>
                    <PaginationEllipsis />
                  </PaginationItem>
...
// const Workout = () => {
//   return (
//     <div className="text-center text-white py-96 text-[50px]">Coming soon</div>
//   );
// };
